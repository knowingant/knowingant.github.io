---
layout: page
title: Handouts and psets
permalink: /handouts/
---

My handouts are of wildly varying quality. Here, they are sorted in order of
decreasing quality. The worst ones (F tier) are not published.
The handouts also have a difficulty rating, measured in chili peppers 🌶.
The target audiences for each rating are described below.
- No peppers: anyone.
- 🌶: MATHCOUNTS State qualifiers.
- 🌶🌶: students that can comfortably score 5 on the AIME.
- 🌶🌶🌶: students comfortable with proofs.

Roughly speaking, 🌶 is the target difficulty for MNYMO,
🌶🌶 is the maximum difficulty acceptable for ARML practices,
and 🌶🌶🌶 is both the target and maximum acceptable difficulty
for the high school group of Math + Science Club.

Some of these (the "problem sets") are meant to accompany a lecture,
so there may not be enough commentary for some handouts.
Also, the dates are messed up because I overuse the `\today` command.

Please let me know about any errors, if you have any questions, or if you want solutions.

# Recorded presentations

- **Penguins**
[(pdf)](stack-sorting/stack-sorting.pdf) [(tex)](stack-sorting/stack-sorting.tex) [(youtube)](https://youtu.be/4Pg7ZQRtXYY)

    A ten-minute RSI talk on stack-sorting
    meant for an audience without a math background.
    You should watch the YouTube video instead of reading the slides;
    credits to Tristan Sun for filming.

- **Tractor**
[(pdf)](tractor/tractor.pdf) [(tex)](tractor/tractor.tex) [(youtube)](https://youtu.be/M2Z3jXubzKM)

    Slides on how to play the card game Tractor, originally made for RSI 2026.

# Handouts

### S tier

- **Complex bashing** 🌶🌶🌶
[(pdf)](complex-bashing/complex-bashing.pdf) [(tex)](complex-bashing/complex-bashing.tex)

    Intended for intermediate complex bashers who are able to complete BGW-complex.
    We also introduce the Jacobi bialternant formula.
    The main content of this handout is 11 walkthroughs.

- **Estimation** 🌶–🌶🌶
[(pdf)](estimation/estimation.pdf) [(tex)](estimation/estimation.tex)

    A beginner's guide to Guts-style estimation.

### A tier

- **Algebraic number theory** 🌶🌶🌶
[(pdf)](algebraic-nt/algebraic-nt.pdf) [(tex)](algebraic-nt/algebraic-nt.tex)

    Notes on basic algebraic number theory.
    Written for the last Math + Science Club lecture of the 2025–26 school year.
    Includes a geometry problem. To be read after Demystifying Finite Fields.

- **Demystifying finite fields** 🌶🌶🌶
[(pdf)](finite-fields/finite-fields.pdf) [(tex)](finite-fields/finite-fields.tex)

    Finite fields of arbitrary order using as little theory as possible.
    We mostly cover the Frobenius automorphism. To be read after Orders.

### B tier

- **Avoiding sillies**
[(pdf)](avoiding-sillies/avoiding-sillies.pdf) [(tex)](avoiding-sillies/avoiding-sillies.tex)

    An anti-silly presentation I originally gave to Minnetonka Math Team.
    Includes scratch paper samples for you to analyze.

- **Orders** 🌶🌶
[(pdf)](orders/orders.pdf) [(tex)](orders/orders.tex)

    A heavily revised version of a handout I wrote for my freshman-year crush.
    Don't ask.

- **erm what the** <script type="math/tex">\sigma</script> 🌶🌶
[(pdf)](nt-sums/nt-sums.pdf) [(tex)](nt-sums/nt-sums.tex)

    An introduction to the Dirichlet convolution and multiplicative functions.
    Originally written for Minnesota ARML.

- **Quaternions** 🌶🌶–🌶🌶🌶
[(pdf)](quaternions/quaternions.pdf) [(tex)](quaternions/quaternions.tex)

    Number theory over the quaternions to prove Lagrange's four-square theorem.
    As warmups, we prove the analagous facts in the integers and the Gaussian integers,
    then finally move to the Hurwitz integers.

- **The title of this handout** 🌶–🌶🌶
[(pdf)](recursion/recursion.pdf) [(tex)](recursion/recursion.tex)

    Solving recurrences explicitly using characteristic polynomials.
    Originally written for Minnesota ARML.

### C tier

- **A taste of Yau Mathcamp** 🌶🌶🌶
[(pdf)](discrepancy-theory/discrepancy-theory.pdf) [(tex)](discrepancy-theory/discrepancy-theory.tex)

    This is a writeup of the most beautiful proof I saw at Yau Mathcamp;
    the topic is discrepancy theory.

- **Hensel's lemma** 🌶🌶
[(pdf)](hensels-lemma/hensels-lemma.pdf) [(tex)](hensels-lemma/hensels-lemma.tex)

    A short lecture on Hensel's lemma I gave during an unofficial
    Minnetonka Math Team meeting after the end of the 2024–25 school year.

### D tier

- **A special kind of polynomial problem** 🌶
[(pdf)](interpolate/interpolate.pdf) [(tex)](interpolate/interpolate.tex)

    A mini-lecture I gave to the Minnetonka Middle School East's
    Math + Science Club when I was a freshman.

- **Dynamic Planet 2024–25 notes**
[(pdf)](dynamic-planet/dynamic-planet.pdf) [(tex)](dynamic-planet/dynamic-planet.tex)

    A binder on glaciers I wrote for Science Olympiad
    and later presented at Math + Science Club.

# Problem sets

- **Bijections** 🌶–🌶🌶
[(pdf)](bijections/bijections.pdf) [(tex)](bijections/bijections.tex)

    The art of using bijections to prove combinatorial identities.
    The original material was produced for Math + Science Club,
    then glued together to produce this problem set for Minnesota ARML.

- **Modular arithmetic** 🌶–🌶🌶🌶
[(pdf)](modular-arithmetic/modular-arithmetic.pdf) [(tex)](modular-arithmetic/modular-arithmetic.tex)

    A modular arithmetic problem set I prepared for Minnesota ARML.
    The set is divided into Level 1, Level 10, Level 100, and Level 1000 problems.

- **Trig from Complex** 🌶–🌶🌶
[(pdf)](complex-trig/complex-trig.pdf) [(tex)](complex-trig/complex-trig.tex)

    A lecture I gave to MN Youth Math Outreach in 2026.

- **Vieta's formulas** 🌶–🌶🌶
[(pdf)](vieta/vieta.pdf) [(tex)](vieta/vieta.tex)

    Everyone and their grandma wrote a Vieta handout at some point.
    Here's mine, which I originally presented at Math + Science Club in 2023.

- **The geometric interpretation** 🌶🌶
[(pdf)](geometric-interpretation/geometric-interpretation.pdf) [(tex)](geometric-interpretation/geometric-interpretation.tex)

    Turn algebra problems into geometry! I usually do the reverse...

- **Generating functions** 🌶🌶–🌶🌶🌶
[(pdf)](genfunc/genfunc.pdf) [(tex)](genfunc/genfunc.tex)
    
    An introduction to generating functions I presented at Math + Science Club.
...
\begin{quote}
	\sffamily\small
	arml estimathon was the second time at a competition where my team loses first place to minnesota in the last \til10 minutes with the first being cmimc

	\medskip

	---\href{https://artofproblemsolving.com/community/q3h3332858p30875431}{MathWizard10 on AoPS}
\end{quote}
...
In this handout, we discuss the ``mathematical'' sort of estimation problems found on Guts sets. There are also metaproblems and a few weird ones involving convex hulls of the schools of the organizers, but we shall ignore those. To get better at those, try Fermi Questions (rip).

Throughout, we use the convention that $A$ is the actual answer and $E$ is the estimate. Additionally, $\log$ denotes the natural logarithm unless explicitly stated.
...
Use of logarithms allows one to quickly compute otherwise-difficult expressions, especially large products. A quote often attributed to Laplace: logarithms, ``by shortening the labors, doubled the life of an astronomer.'' You should memorize at least $\log 2$ and $\log 10$:
...
Here are two nice applications of Stirling's approximation:

\begin{example}[MNYMO 2024/Guts Shortlist]
...
\begin{example}[HMMT 2015/Guts 33]
...
\begin{solution}
    ...
    \[\log(n!) \approx \log\left(\sqrt{2\pi n}\left(\frac ne\right)^n\right) \approx \log\left(\left(\frac ne\right)^n\right) = n(\log n - \log e),\]
    so
    \begin{align*}
...
\begin{remark*}
    ...
    \[\sqrt{2\pi n}\left(\frac ne\right)^n \approx \left(\frac ne\right)^n.\]
    This looks terrible, but it's all inside a $\log$, so we are really only dropping the $\log\left(\sqrt{2\pi n}\right)$ term, which is pretty small.
\end{remark*}
...
\begin{example}[Coupon collector]
	Michael is reading \emph{Big Nate} comics. Every second, he clicks the ``random'' button to view a random comic. Suppose there are $n$ comics. Estimate the expected number of seconds it will take for him to view all of them at least once.
\end{example}
...
\begin{example}[HMMT 2020/Guts 33]
	Estimate
...
This example shows the importance of \emph{looking at the scoring}. The scoring function $\floor{22\min(A/E,E/A)}$ is much more forgiving than, say, $\floor{22\min(A/E,E/A)^4}$. So we could use very rough estimates such as $16 \approx 10\log 5$ in the exponent of $e$.

\begin{example}[HMMT 2015/Guts 34]
...
It's often useful to think of the primes as being ``random'': the ``probability'' of $n$ being prime can be thought of as $\frac1{\log n}$.
...
\begin{remark*}
    ...
    It feels somewhat weird to ``rig'' the solution this way. It's a much better decision to keep the $n \log n \log \log n$ term, because the scoring has an exponent of $8$, which is extremely unforgiving. But as it turns out, dropping the term yields a much better estimate, due to luck. If instead the answer had been closer to $150000$, would the organizers still use the above estimate? On a contest, you are not given the luxury of knowing what the answer is. Contest organizers should write solutions for estimations before they compute the exact answer.

	The astute reader may note that the first $\frac{2027}{\log 2027} \approx 266$ primes less than $2027$ will all be in distinct residue classes modulo $2027$; therefore, our assumption that the primes are randomly distributed modulo $2027$ is not accurate. However, this effect is far too small to cause a meaningful change in our answer. Indeed, taking this into account would make our answer the $k$th prime, where
...
\begin{example}[SMT 2024/Guts 26]
	...
	You will earn $\max(0,25 - \lceil\frac1{4000}\abs{E - A}\rceil)$ points.
\end{example}
...
\begin{example}[HMMT 2020/Guts 34]
...
\begin{example}[HMMT 2024/Guts 34]
...
This section is pretty technical. For background, skim ``\href{https://knowingant.github.io/handouts/nt-sums/nt-sums.pdf}{erm what the $\sigma$}.'' Make sure you know what the following mean:
\begin{itemize}
	\ii The Dirichlet convolution and the fact that it preserves multiplicativity.
	\ii The multiplicative arithmetic functions $\mathbf1$, $\id$, $d$, $\sigma$, $\varphi$, and $\mu$.
...
\begin{solution}
	We have
	\[\sum_{n \ge 1}\frac{\mu(n)}{n^s}\sum_{m \ge 1}\frac1{m^s} = \sum_{k \ge 1}\frac{(\mu \ast \mathbf1)(k)}{k^s} = 1\]
...
\begin{example}
	Let $n = 10^{100}$. Two integers are randomly selected from $\{1,2,\dots,n\}$. Estimate the probability they are coprime.
\end{example}
...
\begin{solution}
	...
	\begin{align*}
		...
		&= \sum_{d \le n}\left\lfloor \frac nd\right\rfloor^2\mu(d) \approx \sum_{d \le n}\frac{n^2}{d^2}\mu(d) = \frac{6}{\pi^2}n^2,
	\end{align*}
...
\begin{solution}
	Write $d = \mathbf1 \ast \mathbf1$, then the sum becomes
	\[
		\sum_{ab \le x}1
		= \sum_{a \le \sqrt x}\left\lfloor \frac xa\right\rfloor + \sum_{b \le \sqrt x}\left\lfloor \frac xb\right\rfloor - \sum_{a,b \le \sqrt x}1
		\approx 2\left(\frac12 x \log x + \gamma x\right) - x
		= x \log x + (2\gamma - 1)x. \qedhere
	\]
\end{solution}

In the above example, we also could have simply evaluated the sum
\[\sum_{ab \le x}1 = \sum_{a \le x}\sum_{b \le x/a}1 = \sum_{a \le x}\left\lfloor \frac xa\right\rfloor \approx x \log x.\]
...
\begin{example}[MNYMO 2024/Guts 27]
	The MN Youth Math Outreach Board has birthdays $B_1,B_2,\dots,B_7$ when written in MMDDYYYY form. For example, if Taylor Swift was the eighth member of the MN Youth Math Outreach Board, $B_8$ would be $12131989$, since her birthday is December 13, 1989. Estimate the sum of the pairwise $\gcd$s of the $B_i$, or
...
\begin{example}[HMNT 2023/Guts 36]
	...
	You will earn $\max\left(0,\left\lceil{20\left(1 - \lvert\log_{10}(A/E)\rvert^{1/5}\right)}\right\rceil\right)$ points.
\end{example}
...
\begin{solution}
	...
	\begin{itemize}
		...
		\ii Drops all $b$ from nonsquarefree $n$. This is larger but still relatively insignificant.
	\end{itemize}
...
\begin{example}[MNYMO 2024/Guts Shortlist]\label{exa:ryan_stats}
	Ryan Niu flips $1000$ coins. Estimate the probability he gets between $490$ and $510$ heads, inclusive.
...
\begin{solution}
	...
	\[X := X_1 + \dots + X_{1000}\]
	is between $-20$ and $20$, inclusive. We're going to approximate $X$ as a continuous variable; since $X$ can only take even values, we're going to fudge the interval $[-20,20]$ to $[-21,21]$.

	Let $Y$ be any random variable. From AP Statistics, recall the following definitions:
	\begin{itemize}
    	\ii The \emph{expected value} or \emph{mean} of a random variable $\mu \defeq \EE(Y)$.
    	\ii The \emph{variance} of a random variable $\Var(Y) \defeq \EE\big((Y - \EE(Y))^2\big) = \EE(Y^2) - \EE(Y)^2$.
            \ii The \emph{standard deviation} $\sigma$ of a random variable is the square root of its variance.
	\end{itemize}
    Additionally, recall that variance is additive: if $X$ and $Y$ are independent random variables, $\Var(X + Y) = \Var(X) + \Var(Y)$. (The proof is just linearity of expectation.)
...
\begin{example}[HMMT 2023/Guts 35]
...
\begin{solution}
	Let $X_i$ be a random variable that chooses $\cbrt{F_i},-\cbrt{F_i}$ at random. Let $X = X_2 + \dots + X_{31}$; we want to find the probability $X$ is in the interval $[-1,1]$. By the central limit theorem, $X$ is approximately normal. Now by Binet the variance of $X$ is
...
\begin{example}[HMMT 2015/Guts 35]
...
\begin{solution}
    ...
    \[\frac{f(11)}{\sum_{i = 0}^{23}f(i)} \approx \frac{f(11)}{\sum_{i = 9}^{14}f(i)} = \frac1{2(1 + 2^{-2} + 2^{-6})} = \frac{32}{81} \approx 0.395.\]
...
\begin{example}[HMMT 2013/Guts 33]
...
\begin{example}[SMT 2024/Guts 25]
	...
	You will earn $\max(0,25 - \lceil 500\abs{E - A}\rceil)$ points.
\end{example}
...
\begin{example}[HMNT 2023/Guts 35]
...
\begin{problem}[BMT 2021/Guts 25]\label{pro:bmt_1}
...
\begin{problem}[BMT 2021/Guts 26]\label{pro:bmt_2}
...
\begin{problem}[BMT 2021/Guts 27]\label{pro:bmt_3}
...
The solutions are on the next page. Don't look at them until you've spent at least 20 minutes.
...
\begin{proof}[Solution to \Cref{pro:bmt_3}]
    ...
    \[\frac{(\log 2 - \log 1) + (\log 4 - \log 3) + \dots + (\log 10 - \log 9)}{\log 10} \approx 0.596.\]
    The difference between the number of even and odd digits is around $2021((1 - 0.596) - 0.596) \approx -388$, so our estimate is $2019 - 388 = 1631$. Since the answer is $1776$, we earned $22$ points.
\end{proof}

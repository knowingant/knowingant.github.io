...
\epigraph{this explains so much about angie}{Aster after finding out that Math Prize for Girls is 90\% trig}

\section{Theory}

Complex numbers are vectors.
\begin{example}[HMMT 2018/A6]
    If
    \[\cos\alpha + \cos\beta + \cos\gamma = \sin\alpha + \sin\beta + \sin\gamma = 1,\]
    compute the smallest possible value of $\cos\alpha$.
\end{example}

\begin{example}[AIME I 2025/8]
...
\begin{example}
    Show that
    \[\tan(89.5\dg) = \sin 1\dg + \sin 2\dg + \dots + \sin 179\dg.\]
\end{example}

\begin{example}[CMIMC 2018/A7]
    Compute
...
Note that any M\"obius transformation is bijective. Now what do these two definitions have to do with each other? Well, we have the following nuclear weapon.
...
\begin{problem}
    Derive the tangent addition identity $\tan(\alpha + \beta) = \frac{\tan(\alpha) + \tan(\beta)}{1 - \tan(\alpha)\tan(\beta)}$ with complex numbers.
\end{problem}

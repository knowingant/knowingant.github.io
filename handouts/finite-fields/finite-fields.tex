...
Finite fields are a pretty advanced topic, but they come up in short-answer contests surprisingly often. My first exposure to a finite field of nonprime order was \Cref{exa:cubic_discriminant}. The solution was not understandable to me, since I barely knew what a field was. I asked Evan Chen, whose answer was essentially ``read section 60 of \emph{Napkin},'' which was not so helpful since section 60 depends on sections 1, 3 through 5, and 53 through 59, for a total of 12 sections.
...
The goal here is to explain finite fields using as little theory as possible, which comes at the expense of leaving many facts unproved, but allows the reader to learn through examples. In particular, shoot me if I say ``automorphism group.'' We leave only the parts that are relevant to contests; however, we still require experience with modular arithmetic, especially orders, Fermat's little theorem, and modular inverses. If you are unfamiliar with the above, see my orders handout, which may be found on \href{https://knowingant.github.io/handouts/orders/orders.pdf}{my website}.

\section{What is a field?}

A field is a set where you can add, subtract, multiply, and divide (except by zero), like $\QQ$, $\RR$, and $\CC$. Here, we will work with \emph{finite fields}, which (as their name suggests) have a finite number of elements, such as the integers modulo a prime $p$. The field with $p$ elements is called $\FF_p$ or $\ZZ/p\ZZ$. If you want the precise definition of a field, here it is, but beware: it is boring.
...
\begin{example}[BMT 2021/General 21]\label{exa:motivation}
...
\begin{solution}{\Cref{exa:motivation}}
    ...
    \begin{itemize}
        ...
        \ii Let $\FF_p[x]$ be the set of polynomials with coefficients in $\FF_p$.\footnote{Note that $\FF_p[x]$ is the set of \emph{polynomials}, not the set of polynomial functions. For example, let $P_1(x) = x^2 + x$ and $P_2(x) = 0$. Then $P_1$ and $P_2$ are different polynomials in $\FF_2[x]$, even though $P_1(x) = P_2(x) = 0$ for all $x \in \FF_2$.} For $P \in \FF_p[x]$, $\sigma(P(x)) = P(\sigma(x))$: write $P(x) = a_nx^n + a_{n - 1}x^{n - 1} + \dots + a_0x^0$, then
        \begin{align*}
...
\begin{example}[BMT 2023/Discrete 10]\label{exa:bprp}
...
\begin{solution}{\Cref{exa:bprp}}
    ...
    A computation with quadratic reciprocity shows $(\frac{17}{997}) = -1$, so $\sqrt{17} \not\in \FF_p$. Now we work in the field
...
\begin{example}[HMMT 2017/A9]\label{exa:fibonacci}
...
\begin{solution}{\Cref{exa:fibonacci}}
    ...
    \end{align*}
    so $F_{p + 2} = F_{p + 1} + F_p = -1$ and $F_{2p + 3} = F_{2p + 2} + F_{2p + 1} = 1$. Therefore
    \[(F_0,F_1) = (F_{256},F_{257}) \neq (F_{128},F_{129}),\]
    so the period divides $256$ but not $128$, so it is exactly $256$.
\end{solution}

\begin{example}[HMMT 2023/Guts 31]\label{exa:cubic_discriminant}
...
\begin{solution}{\Cref{exa:cubic_discriminant}}
    ...
    Since $a,b,c \not\in \FF_p$ and they are all roots of $P$, $(a^p,b^p,c^p)$ is a nontrivial cyclic permutation of $(a,b,c)$, so this becomes
    ...
    so
    \[P'(a) = (a - b)(a - c) \qquad P'(b) = (b - c)(b - a) \qquad P'(c) = (c - a)(c - b).\]
    Therefore
    \begin{align*}
...
\begin{exercise}[BMT 2022/Discrete 8]

...
\begin{definition}
	...
	\[\ZZ/20\ZZ = \{0,1,\dots,19\} \text{ and } (\ZZ/20\ZZ)^\times = \{1,3,7,9,11,13,17,19\}.\]
\end{definition}
...
Here are some nonexamples of groups.
\begin{itemize}
...
\begin{definition}
	An \emph{isomorphism} between two groups $(G,\star)$ and $(H,\circ)$ is essentially a structure-preserving bijection. That is, a function $f: G \to H$ is an isomorphism if:
	\begin{itemize}
		\ii $f$ is bijective.
		\ii For all $g_1,g_2 \in G$, $f(g_1 \star g_2) = f(g_1) \circ f(g_2)$.
	\end{itemize}
...
\begin{definition}
	...
	\[(g_1,h_1) \ast (g_2,h_2) = (g_1 \star g_2,h_1 \circ h_2).\]
...
\[f(1) = (0,0)\text{, }f(3) = (1,0)\text{, }f(5) = (0,1)\text{, and }f(7) = (1,1).\]
...
\[3^{1000} \equiv 3^{22 \cdot 45 + 10} \equiv (3^{22})^{45} \cdot 3^{10} \equiv 3^{10} \equiv 8 \pmod{23}.\]

Now it is time to introduce the notion of orders.

\begin{definition}
	The \emph{order} of an element $a \in (\ZZ/p\ZZ)^\times$, denoted $\ord_p(a)$, is the smallest positive integer $m$ such that $a^m \equiv 1 \pmod{p}$.
\end{definition}
...
\begin{proof}
	Suppose otherwise, and write $n = q\ord_p(a) + r$ where $0 < r < \ord_p(a)$. Now
	\[a^n \equiv \left(a^{\ord_p(a)}\right)^q \cdot a^r \equiv a^r \equiv 1 \pmod{p},\]
...
\begin{theorem}[Primitive roots generate $(\ZZ/p\ZZ)^\times$]\label{thm:prim_roots_generate}
	...
	\[(\ZZ/p\ZZ)^\times = \{1,2,\dots,p - 1\} = \{g^0,g^1,\dots,g^{p - 2}\}.\]
\end{theorem}
...
\begin{theorem}\label{thm:cyclic}
	We have
	\[((\ZZ/p\ZZ)^\times,\cdot) \cong (\ZZ/(p - 1)\ZZ,+).\]
\end{theorem}
...
\begin{example}[Weakest HMMT 2019/Guts 28]
	For how many integers $a$ with $1 \le a \le 41$ is the sequence $a^1,a^2,a^4,a^8,\dots$ eventually constant $\pmod{41}$?
\end{example}

We see $41$ works; now we count the $a \in (\ZZ/41\ZZ)^\times$. Let $a = g^k$. We want the sequence $g^k,g^{2k},g^{4k},\dots$ to eventually converge, which is equivalent to the exponents $k,2k,4k,\dots$ converging $\pmod{40}$. This occurs if and only if $k$ is a multiple of $5$, of which there are $8$. Therefore, the answer is $9$.

\begin{example}[$n^2 + 1$ lemma]
	Let $p$ be an odd prime. Then there exists an $n$ such that $p \mid n^2 + 1$ if and only if $p \equiv 1 \pmod{4}$.
\end{example}
...
Since $2024^2 = 2^6 \cdot 11^2 \cdot 23^2$, the only primes dividing $a^2 + b^2$ are $2$, $11$, and $23$. Let $a = xd$ and $b = yd$ with $x$ and $y$ relatively prime. Then $x = y = 1$, else there's a $1 \bmod 4$ prime dividing $x^2 + y^2$ and hence $2024$. Then $2d^2 \mid 2024^2$, so $d \mid 4 \cdot 11 \cdot 23$, so the answer is $12$.

\begin{example}
	How many primitive roots are there $\pmod{p}$?
\end{example}

Translating the problem into $\ZZ/(p - 1)\ZZ$, we want to count the $a \in \ZZ/(p - 1)\ZZ$ such that $a,2a,\dots,(p - 2)a$ are nonzero $\pmod{p - 1}$. This occurs if and only if $a$ is relatively prime to $p - 1$, so there are $\varphi(p - 1)$ primitive roots.
...
Groups that are isomorphic to $(\ZZ/n\ZZ,+)$ for some $n$ are called \emph{cyclic}. We saw previously that $((\ZZ/p\ZZ)^\times,\cdot)$ is cyclic. It turns out there exist primitive roots modulo $n$ if and only if $n$ is of the form $2,4,p^k,2p^k$ where $p$ is an odd prime and $k$ is a positive integer. In other words, $(\ZZ/n\ZZ)^\times$ is cyclic, or,
\[((\ZZ/n\ZZ)^\times,\cdot) \cong (\ZZ/\varphi(n)\ZZ,+).\]
Here, the idea of a primitive root is generalized to mean
\[g^1,g^2,\dots,g^{\varphi(n) - 1}\]
are all not congruent to $1 \pmod{n}$.

\begin{example}[Weak HMMT 2019/Guts 28]\label{ex:weak_hmmt}
...
\begin{example}[HMMT 2019/Guts 28]
...
This is the same as the earlier problems but asking for $100$ instead of $41$ or $25$. The key insight here is that we can split $\ZZ/100\ZZ$ into $\ZZ/4\ZZ$ and $\ZZ/25\ZZ$ by the Chinese remainder theorem:
\begin{itemize}
	\ii In $\ZZ/25\ZZ$, there are $9$ working residues by \Cref{ex:weak_hmmt}.
	\ii In $\ZZ/4\ZZ$, clearly all $4$ residues work.
\end{itemize}
Each pair of working residues in $\ZZ/25\ZZ$ and $\ZZ/4\ZZ$ uniquely corresponds to a working residue in $\ZZ/100\ZZ$, again by the Chinese remainder theorem, so the answer is $9 \cdot 4 = 36$.
...
\begin{align*}
	...
	&\cong (\ZZ/2\ZZ)^4 \times \ZZ/5\ZZ \times \ZZ/11\ZZ.
\end{align*}
...
\begin{example}[PUMaC A 2016/N7]
	For how many integers $n$ with $2017 \le n \le 2017^2$ do we have $n^n \equiv 1 \pmod{2017}$?
\end{example}

Observe $p = 2017$ is prime. Since $n = 2017^2$ fails, we replace the interval with $S = [2017,2017^2) \cap \ZZ$. Now the value of $n^n \pmod{p}$ is uniquely determined by $n \pmod{p}$ and $n \pmod{p - 1}$. Also, as $n$ ranges over $S$, its remainders modulo $p - 1$ and $p$ take every value $(r_1,r_2) \in \ZZ/(p - 1)\ZZ \times \ZZ/p\ZZ$ exactly once. Therefore, we just want the number of ordered pairs $(a,b) \in \ZZ/p\ZZ \times \ZZ/(p - 1)\ZZ$ such that $a^b \equiv 1 \pmod{p}$.

By \Cref{ex:num_order}, there are $\varphi(d)$ elements of order $d$ for each $d \mid p - 1$. For each element $a \in (\ZZ/p\ZZ)^\times$, there are $\frac{p - 1}{\ord_p(a)}$ possible $b$ such that $a^b \equiv 1 \pmod{p}$. Summing over $a$, our answer is
\[\sum_{a = 1}^{p - 1}\frac{p - 1}{\ord_p(a)}.\]
...
\[\left(2^5 + 5(2^5 - 2^4)\right)\left(3^2 + 2(3^2 - 3)\right)\left(7 + 7 - 1\right) = 30576.\]

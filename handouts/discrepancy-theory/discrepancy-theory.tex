...
The goal of this lecture is to prove the following beautiful theorem, which was one of the first results in the field of \emph{discrepancy theory}, which studies how ``evenly distributed'' sequences are.
...
Buckle your seatbelts. This proof is going to be a hike, but I promise the view is worth it. There are four versions of \Cref{thm:sqrt-log-N}. Here they are in order of increasing difficulty.
\begin{itemize}
    \ii Replace $\sqrt{\log N}$ with $1$. This one is trivial.
    \ii Replace $\sqrt{\log N}$ with $\frac{\log\log N}{\log\log\log N}$, proven by van Aardenne-Ehrenfest in 1945.
    \ii The original \Cref{thm:sqrt-log-N}, proven by Roth in 1954.
    \ii Replace $\sqrt{\log N}$ with $\log N$, proven by Schmidt in 1972. This is best possible.
\end{itemize}

We will prove \Cref{thm:sqrt-log-N}. We claim the following lemma, which finishes the proof when applied to the sequence $(P_i,\frac{i - 1}N)_{1 \le i \le N}$.
...
Let $c = c_1$ be some constant to be determined later. We begin with some definitions. For $x = (x_1,x_2) \in [0,1)^2$, let
...
Let $R = [0,1)^2$. A \emph{rectangle} is a subset of $R$ of the form $[a,b) \times [c,d)$ for some real $a,b,c,d$ with $a < b$ and $c < d$. For a rectangle $r = [a,b) \times [c,d)$, we define the four subrectangles
\[
    \begin{array}{cc}
...
\begin{align*}
    ...
    &= -N\lvert a\rvert^2\lvert b\rvert^2 = -\frac N{2^{2m}}.
\end{align*}
By pigeonhole, there are at least $N$ good rectangles in each $\mathscr P_i$, so we bound
\begin{align*}
    \biggl( \int_RD(x)F(x) \odif x \biggr)^2 &= \biggl( \sum_{i = 0}^m\sum_{\text{good $r \in \mathscr P_i$}}\int_rD(x)f_i(x) \odif x \biggr)^2 \\
    &= \biggl( \sum_{i = 0}^m\sum_{\text{good $r \in \mathscr P_i$}}\frac N{2^{2m}} \biggr)^2 \\
    &\ge \frac{N^4(m + 1)^2}{2^{4m}}.
\end{align*}

Combining the bounds, we obtain
\[\int_R D(x)^2\odif x \ge \frac{(\int_R D(x)F(x) \odif x)^2}{\int_R F(x)^2 \odif x} \ge \frac{N^4(m + 1)}{2^{4m}} \ge \frac1{2^7} + \frac{\log N}{2^8\log 2},\]
as desired.

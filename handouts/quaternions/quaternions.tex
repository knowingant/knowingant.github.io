...
\begin{frame}
	...
	\begin{itemize}
		...
		\ii Let $(m_0,n_0) = (m,n)$.
        \ii For $i \ge 0$, let $(m_{i + 1},n_{i + 1}) = (n_i,m_i - k_in_i)$ where $k_i$ is chosen such that $\abs{m_i - k_in_i} < \lvert n_i\rvert$.
...
\begin{frame}
	...
	\begin{proof}
		Suppose $p \mid mn$ and $p \nmid m$. Then $\abs{\gcd(p,m)} = 1$, so by B\'ezout's identity there are Gaussian integers $a$ and $b$ such that $ap + bm = \gcd(p,m)$, or equivalently, multiplying by $\overline{\gcd(p,m)}$, there are Gaussian integers $a'$ and $b'$ such that $a'p + b'm = 1$. So $a'pn + b'mn = n$, but since $p \mid a'pn + b'mn$, $p \mid n$.
	\end{proof}
\end{frame}
...
\begin{frame}
	...
	\begin{theorem}[Fermat's Christmas theorem]
		Let $p \in \ZZ$ be a prime. Then $p = x^2 + y^2$ for some $x,y \in \ZZ$ if and only if $p = 2$ or $p \equiv 1 \pmod 4$.
	\end{theorem}
...
\begin{frame}
	...
	\begin{itemize}
		\ii For $d,h \in H$, we say $d$ \emph{right divides} $h$, written $d \rdiv h$, if $h = ad$ for some $a \in H$. We say $d$ \emph{left divides} $h$, written $d \ldiv h$, if $h = db$ for some $b \in H$.\footnote{I just made up the notation $\rdiv$ and $\ldiv$ for right and left divisibility. I'm not sure if any standard notation exists.}
		\ii A Hurwitz integer $p$ is called \emph{prime} if $d \rdiv p$ implies $\abs d = 1$ or $\abs d = \abs p$.
	\end{itemize}

	We require two notions of divisibility because multiplication is not commutative.

	We will prove the following. Look familiar?
...
\begin{frame}
	...
	\begin{block}{}
		For all $x \in \HH$, there is some $h \in H$ such that $\abs{x - h} < 1$.
	\end{block}
...
\begin{frame}
	...
	\begin{proof}
		Suppose $p \mid mn$ and $p \nmid m$. Then $\abs{\gcd(p,m)} = 1$, so by B\'ezout's identity there are Hurwitz integers $a$ and $b$ such that $ap + bm = \gcd(p,m)$, or equivalently, left multiplying by $\gcd(p,m)^*$, there are Hurwitz integers $a'$ and $b'$ such that $a'p + b'm = 1$. So $a'pn + b'mn = n$, but since $p \mid a'pn + b'mn$, $p \mid n$.
	\end{proof}
\end{frame}
...
\begin{frame}
	\frametitle{The cross product and quaternions}
	Consider the quaternions with no real part $x = x_1i + x_2j + x_3k$ and $y = y_1i + y_2j + y_3k$. Then if the imaginary part of $xy$ is $z_1i + z_2j + z_3k$, then
...
\begin{frame}
	...
	\begin{figure}
		...
		\caption{The octonion multiplication table with $e_0 = 1$ and the imaginary units $e_1,\dots,e_7$. From Wikipedia, to avoid formatting a table in \LaTeX{}.}
	\end{figure}
\end{frame}

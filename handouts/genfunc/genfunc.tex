...
\begin{example}
    Is it possible to relabel the faces of a pair of six-sided dice with positive integers such that the probability of rolling each sum from $2$ to $12$ stays the same?
\end{example}
...
\begin{problem}[HMMT 2007/C9]
    Let $S$ denote the set of triples $(i,j,k)$ of positive integers where $i + j + k = 17$. Compute
    \[\sum_{(i,j,k) \in S}ijk.\]
\end{problem}

\begin{problem}[Project Euler 956]
    Let $\Omega(n)$ be the number of prime factors of $n$, counted with multiplicity. Let $D(n,m)$ be the sum of all divisors $d$ of $n$ where $\Omega(d)$ is divisible by $m$. Describe an efficient algorithm to compute

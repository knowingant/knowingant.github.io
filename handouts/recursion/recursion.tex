...
\begin{example}[Binet's formula]
    ...
    \[F_{n} = \frac{1}{\sqrt{5}}\left(\left(\frac{1 + \sqrt5}2\right)^n - \left(\frac{1 - \sqrt5}2\right)^n\right).\]
\end{example}

We could prove this formula using induction or something similar, but we will instead show how this was derived.

Our approach is to first ignore the initial conditions $F_0 = 0$ and $F_1 = 1$
and instead focus on characterizing \emph{all} sequences $a$ that satisfy the condition
\[a_n = a_{n - 1} + a_{n - 2}. \qquad (\bigstar)\]
Observe that the set of sequences satisfying $(\bigstar)$
is closed under \emph{linear combinations}. 
That is, if sequences $u$ and $v$ satisfy $(\bigstar)$,
for any real numbers $a$ and $b$, $au + bv$ satisfies $(\bigstar)$ as well,
where scalar multiplication and addition are defined component-wise.
Buzzword: the set of sequences satisfying the Fibonacci relation forms a vector space with dimension $2$.

The big leap here is to guess two ``nice'' sequences $u$ and $v$ that satisfy the Fibonacci relation.
Then, we hope we can find $a$ and $b$ such that $au + bv$ has zeroth term $0$ and first term $1$, which gives the Fibonacci sequence.
We guess the geometric sequences $u_n = \alpha^n$ and $v_n = \beta^n$ for some $\alpha$ and $\beta$. Then the Fibonacci relations become
\begin{align*}
...
\end{align*}
So $\alpha$ and $\beta$ are roots of the polynomial $P(\lambda) = \lambda^2 - \lambda - 1$.
We call $P$ the \emph{characteristic polynomial} of the recurrence relation $a_n - a_{n - 1} - a_{n - 2} = 0$.
It's not a coincidence that the coefficients of the characteristic polynomial correspond with the coefficients of the recurrence relation. Similarly, the characteristic polynomial of the recurrence relation $a_n = 3a_{n - 1} - 2a_{n - 3}$ (or $a_n - 3a_{n - 1} + 2a_{n - 3} = 0$) is $\lambda^3 - 3\lambda^2 + 2$.

\begin{exercise}
    What is the characteristic polynomial of the recurrence relation $a_n = 4a_{n - 2} - a_{n - 3}$?
\end{exercise}

The roots of the characteristic polynomial correspond to the geometric sequences that satisfy the recurrence relation. Going back to the Fibonacci sequence, the roots of the characteristic polynomial $\lambda^2 - \lambda - 1$ are $\alpha = \frac{1 + \sqrt5}2$ and $\beta = \frac{1 - \sqrt5}2$ from the quadratic formula. Therefore $u_n = \alpha^n$ and $v_n = \beta^n$.
...
\[F_n = au_n + bv_n = \frac{1}{\sqrt{5}}\left(\left(\frac{1 + \sqrt5}2\right)^n - \left(\frac{1 - \sqrt5}2\right)^n\right),\]
as desired.
...
\begin{problem}[HMMT 2012/Guts 2]
    \label{pro:hmmt-guts}
...
\begin{problem}
    ...
    Find the number of ways to color each cell of a $2 \times 6$ grid black or white such that no $2 \times 2$ square is completely black.
\end{problem}

\begin{problem}[CMIMC 2023/A Estimation]
...
\begin{solution}{\Cref{pro:trivial}}
    ...
    Define the sequence $b$ such that $a_n = b_n + c$ for some constant $c$. Now the condition becomes $b_n + c = 3(b_{n - 1} + c) + 1$, which rearranges to $b_n = 3b_{n - 1} + 2c + 1$. Since we want to yeet (cancel out) the constant term, we set $c = -\frac12$. Now $b_0 = a_0 + \frac12 = \frac12$ and $b_n = 3b_{n - 1}$. This is a geometric sequence, so $b_n = \frac{3^n}{2}$, and $a_n = \frac{3^n}{2} - \frac{1}{2}$.
\end{solution}

\begin{solution}{\Cref{pro:hmmt-guts}}
    Computing the first few terms $a_0 = \frac21$, $a_1 = \frac23$, $a_2 = \frac25$, and $a_3 = \frac27$, it's pretty clear $a_n = \frac{2}{2n + 1}$, so $a_{2012} = \frac{2}{4025}$. You could also prove this by induction or whatever.
\end{solution}

\begin{solution}{\Cref{pro:grid-classic}}
    Casework on this thing would be absolutely disgusting, so we use recursion. Call a coloring \emph{good} if it contains no $2 \times 2$ blocks that are completely black. Call a coloring \emph{xooks} if its rightmost edge is black-black, and \emph{rbo} otherwise. Let $a_n$ be the number of good xooks colorings of a $2 \times n$ grid, and let $b_n$ be the number of good rbo colorings of a $2 \times n$ grid. We want $a_6 + b_6$.
...
\begin{solution}{\Cref{pro:smt-team}}
    ...
    \begin{align*}
        ...
        a + 4b + 8c &= 3.
    \end{align*}
    Solving, we see $(a,b,c) = (\frac13,-\frac13,\frac12)$, so $Q_n = \frac13(-1)^n - \frac13 \cdot 2^n + \frac12 n \cdot 2^n$, and

...
\begin{abstract}
	This handout is intended for intermediate complex bashers. You should be able to complete \verb|BGW-complex| or solve most of the problems in \Cref{sec:warm-ups}. The main content of this handout is \Cref{sec:walkthroughs}, which contains 11 walkthroughs in rough order of increasing difficulty. You should try these walkthroughs yourself before reading them.
\end{abstract}
...
\begin{itemize}
	...
	\begin{itemize}
		\ii Expressions must always be homogeneous (i.e., of the same degree). For example, $bc(a^2\ol z + b + c)$ is homogeneous of degree $3$, since each term has degree $3$. If you ever see something like $bc(a^2\ol z + 1)$, something has gone horribly wrong.
...
\begin{itemize}
    ...
    \end{align*}
    \ii Let $f,g \in \CC[x]$ and let $h$ be an irreducible quadratic with roots $r$ and $s$. Let $n = \max(\deg f,\deg g) + 1$. Then $\frac{f(r)}{g(r)} = \frac{f(s)}{g(s)}$ if and only if $\alpha f(x) + \beta g(x) + C(x)h(x) = 0$ for some $\alpha$, $\beta$, and $C$ not all zero. Interpreting $\CC[x]$ as a vector space, this holds if and only if the determinant of the $n \times n$ matrix with one row as the coefficients of $f$, one row as the coefficients of $g$, and $n - 2$ rows as the coefficients of $x^ih(x)$ for $0 \le i < n - 2$ is zero. This is probably trivial by resultants or something but I don't know resultants.
...
\[\begin{ytableau}
	...
	1 & 2 & 2 & 7 & 8
\end{ytableau}\]
If $\lambda$ and $\mu$ are two partitions of $n$, define the Kostka number $K_{\lambda\mu}$ to be the number of semistandard Young tableaux of shape $\lambda$ and content $\mu$.

Observe that $S_n$ acts on $\CC[x_1,\dots,x_n]$ by permutation of variables, so for a partition $\lambda$, define
\[a_\lambda = \sum_{w \in S_n}\sgn(w)w(x_1^{\lambda_1}\cdots x_n^{\lambda_n}) = \det(x_i^{\lambda_j})_{1 \le i,j \le n}.\]
We will now define the monomial symmetric functions. Let $\lambda$ be a partition of length $n$ and let $m_i$ be the number of times $i$ appears in $\lambda$. Then
\[m_\lambda(x_1,\dots,x_n) := \frac1{m_0!\cdots m_n!}\sum_{w \in S_n}w(x_1^{\lambda_1} \dots x_n^{\lambda_n}).\]
...
where the sum is taken over semistandard tableaux $\tau$ on $\lambda$ with entries in $\{1,\dots,n\}$. This is equivalent to taking every semistandard tableau $\tau$, replacing each $i$ with $x_i$, multiplying the elements of $\tau$, and summing.

It is a fact that the Schur functions are symmetric, i.e., they are invariant under any permutation of variables. We will sketch the proof, for completeness. The idea is that any permutation is a composition of adjacent transpositions, so it's enough to show $s_\lambda(x_1,\dots,x_i,x_{i + 1},\dots,x_n) = s_\lambda(x_1,\dots,x_{i + 1},x_i,\dots,x_n)$. For this, it's enough to show the number of semistandard tableaux with content $(m_1,\dots,m_i,m_{i + 1},\dots,m_n)$ is equal to the number of semistandard tableaux with content $(m_1,\dots,m_{i + 1},m_i,\dots,m_n)$ for all $(m_1,\dots,m_n)$. To show this, there's a bijection.

\begin{definition}
	For each $k$, the Bender--Knuth involution $\sigma_k$ is an involution on the set of semistandard tableaux fixing shape and content except for swapping the number of occurrences of $k$ and $k + 1$. Specifically, the Bender--Knuth involution $\sigma_k$ is the composition of the following three operations.
	\begin{enumerate}
...
These problems are relatively straightforward and should be tried before the walkthroughs. Many of these are taken from the OTIS unit \verb|BGW-complex|.
...
\begin{problem}[ToT Spring 2018/J-A4]
	...
	Let $P$ be the foot of the perpendicular from $A$ to line $CO$.
	Prove that line $HP$ bisects side $AB$.
\end{problem}

\begin{problem}[PUMaC 2016/Finals A3]
...
\begin{problem}[IMO 2023/2]
	Let $ABC$ be an acute triangle with $AB < AC$. Let $\Omega$ be the circumcircle of $ABC$. Let $S$ be the midpoint of the arc $CB$ of $\Omega$ containing $A$. The perpendicular from $A$ to $BC$ meets $BS$ at $D$ and meets $\Omega$ again at $E \neq A$. The line through $D$ parallel to $BC$ meets line $BE$ at $L$. Denote the circumcircle of triangle $BDL$ by $\omega$. Let $\omega$ meet $\Omega$ again at $P \neq B$. Prove that the line tangent to $\omega$ at $P$ meets line $BS$ on the internal angle bisector of $\angle BAC$.
\end{problem}
...
\begin{walkthrough}
	...
	Let $\omega$ be the unit circle, and let $BC$, $CA$, and $AB$ be tangent to $\omega$ at $X$, $Y$, and $Z$, respectively.
	\begin{walk}
...
\begin{walkthrough}
	...
	Let $\omega$ be the unit circle. Let $\Gamma$ have radius $r$ and be centered at $k \in \RR$. Let $\ell$ be tangent to $\omega$ at $Z$.
	\begin{walk}
...
\begin{example}[RMM 2026/5]
	Let $ABC$ be a triangle with $AB < AC$, let $O$ be its circumcenter and let $XYZT$ be a parallelogram inside triangle $ABC$ such that
	\begin{align*}
...
\begin{example}[ISL 2024 G5]
	Let $ABC$ be a triangle with incenter $I$, and let $\Omega$ be the circumcircle of triangle $BIC$. Let $K$ be a point in the interior of segment $BC$ such that $\angle BAK < \angle KAC$. The angle bisector of $\angle BKA$ intersects $\Omega$ at points $W$ and $X$ such that $A$ and $W$ lie on the same side of $BC$, and the angle bisector of $\angle CKA$ intersects $\Omega$ at points $Y$ and $Z$ such that $A$ and $Y$ lie on the same side of $BC$.
...
\begin{walkthrough}
	...
	We will set $\Omega$ as the unit circle. Let $j$ represent $I$.
	\begin{center}
		\begin{asy}
		/*
...
\begin{walkthrough}
	...
	\begin{theorem*}
		[Casey's theorem]
		Four circles $c_1$, $c_2$, $c_3$, and $c_4$ are tangent to a fifth circle if and only if for some choice of signs,
	...
	\end{walk}
	Why should we have expected to use Ptolemy's theorem a priori?
	...
	\begin{theorem*}
		...
		\begin{itemize}
			...
			\ii given complex numbers $A$, $B$, $C$, $X$, $Y$, and $Z$ such that $A$, $B$, and $C$ are pairwise distinct and $X$, $Y$, and $Z$ are pairwise distinct, there's exactly one M\"obius transformation sending $A$ to $X$, $B$ to $Y$, and $C$ to $Z$.
		\end{itemize}
	\end{theorem*}
	\begin{walk}[resume]
...
\begin{example}[IMO 2013/3]
    Let the excircle of triangle $ABC$ opposite the vertex $A$ be tangent to the side $BC$ at the point $A_1$. Define the points $B_1$ on $CA$ and $C_1$ on $AB$ analogously, using the excircles opposite $B$ and $C$, respectively. Suppose that the circumcenter of triangle $A_1B_1C_1$ lies on the circumcircle of triangle $ABC$. Prove that triangle $ABC$ is right.
\end{example}
...
\begin{walkthrough}
	...
	\begin{walk}[resume]
		...
		\ii Expand in terms of $m_\lambda$ to obtain

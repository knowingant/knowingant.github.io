...
export const exchangeNextPlayer = (state: ExchangePhase): number | null => {
  const kittyTheftEnabled =
    state.propagated.kitty_theft_policy === "AllowKittyTheft";
...
function ExchangeWrapper(props: IExchangeProps): JSX.Element {
  ...
  return <Exchange {...props} send={send} />;
}

interface IExchangeState {
  friends: Friend[];
}
class Exchange extends React.Component<IExchangeInnerProps, IExchangeState> {
  constructor(props: IExchangeInnerProps) {
    super(props);
  ...
  render(): JSX.Element {
    ...
    const nextPlayer = exchangeNextPlayer(this.props.state);

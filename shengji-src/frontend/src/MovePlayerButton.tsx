...
interface Props {
  players: Player[];
  player: Player;
  /// Reordering is locked while a match is in progress (the server rejects
  /// it), so the button stays visible but does nothing.
  disabled?: boolean;
}
...
function MovePlayerButton(
  ...
  const component = (props: Props): JSX.Element => {
    ...
    return (
      <button onClick={movePlayer} disabled={props.disabled}>
        {children}
      </button>
    );
  };

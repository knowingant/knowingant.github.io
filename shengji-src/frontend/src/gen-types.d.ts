...
export type GameMessage =
  ...
  | {
      Joined: {
        /**
         * Seat names controlled by this connection (1 in Standard rooms, 1 or 2 in OneVsOne rooms).
         */
        names: string[];
        player_ids: number[];
...
/**
 * Whether one connection controls one seat (Standard) or two seats on the same team (OneVsOne). This is fixed when the room is created; there is no client action to change it.
 */
export type PlayerMode = "Standard" | "OneVsOne";
 ...
 */
export type RatingMode = "team" | "1v1";
...
export interface PropagatedState {
  ...
  /**
   * Random identifier of the match in progress (set when its first round starts, cleared when it ends). Used by the backend to rate each match exactly once.
   */
  match_key?: string;
  ...
  rated?: boolean;
  /**
   * Random identifier of the round currently being played (set when a round starts).
   */
  round_key?: string;
  should_reveal_kitty_at_end_of_game?: boolean;
  special_decks?: Deck[];
  /**
   * Players who have clicked "start" for the first round of a match. The first round only starts once every player has.
   */
  start_votes?: number[];
...
export interface RatingView {
  /**
   * Matches in which the user neither won nor lost overall (possible in Finding Friends, where several players can reach the target rank).
   */
  draws: number;
  losses: number;
  /**

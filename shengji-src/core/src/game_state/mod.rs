...
use crate::settings::{PlayerMode, PropagatedState};
...
impl GameState {
    ...
    pub fn kick(&mut self, id: PlayerID) -> Result<Vec<MessageVariant>, Error> {
        match self {
            GameState::Initialize(ref mut p) => {
                if p.observers().iter().any(|o| o.id == id) {
                    p.propagated_mut().remove_observer(id)?;
                    return Ok(vec![]);
                }
                let mut msgs = p.remove_player(id)?;
                // A player leaving mid-match voids the match.
                if p.match_in_progress() {
                    p.propagated_mut().reset_match();
                    msgs.push(MessageVariant::MatchAbandoned);
                }
                Ok(msgs)
            }
    ...
    /// Redact the state for a connection which controls all of `ids`.
    ...
    /// The player who has requested a reset of the current game, if any.
    pub fn reset_requester(&self) -> Option<PlayerID> {
        match self {
            GameState::Initialize(_) => None,
            GameState::Draw(p) => p.reset_requester(),
            GameState::Exchange(p) => p.reset_requester(),
            GameState::Play(p) => p.reset_requester(),
        }
    }

    pub fn set_player_mode(&mut self, player_mode: PlayerMode) -> Result<(), Error> {
        match self {
            GameState::Initialize(ref mut p) => p.propagated_mut().set_player_mode(player_mode),
            _ => bail!("player mode can only be set before the game starts"),
        }
    }
}
...
mod tests {
    ...
    fn test_full_game_play() {
        ...
        let mut init = InitializePhase::new();
        // These tests climb past rank 5; keep the old "climb to NT" behaviour.
        init.set_first_to_rank(Rank::NoTrump).unwrap();

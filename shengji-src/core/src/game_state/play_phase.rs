...
impl PlayPhase {
    ...
    pub fn finish_trick(&mut self) -> Result<Vec<MessageVariant>, Error> {
        ...
        // If the bottom holds no points and the remaining cards can't change
        // the outcome, the round is over.
        if !self.game_finished() && !self.kitty_has_points() && !self.outcome_still_open()? {
            self.game_ended_early = true;
            msgs.push(MessageVariant::GameEndedAutomatically);
        }

        Ok(msgs)
    }

    fn kitty_has_points(&self) -> bool {
        self.kitty.iter().any(|c| c.points().is_some())
    }

    /// Whether the remaining unplayed cards could still move the result
    /// across a scoring threshold (uses the true collected points, not the
    /// possibly hidden ones).
    fn outcome_still_open(&self) -> Result<bool, Error> {
    ...
    pub fn finish_game_early(&mut self) -> Result<MessageVariant, Error> {
        ...
        if self.kitty_has_points() {
            bail!("The bottom still has points in it, so the result isn't decided yet");
        }
    ...
    pub fn finish_game(&self) -> Result<(InitializePhase, bool, Vec<MessageVariant>), Error> {
        ...
        if propagated.match_target_reached() {
            // First to rank N: the match is over. Everyone goes back to rank
            // 2 for the next one and waiting observers can join.
            msgs.push(MessageVariant::MatchFinished {
                match_key: propagated.match_key.clone(),
                first_to_rank: propagated.first_to_rank,
                standings: propagated.standings(),
            });
            propagated.reset_match();
    ...
    fn return_to_initialize(&self) -> Result<(InitializePhase, Vec<MessageVariant>), Error> {
        ...
        let mut propagated = self.propagated.clone();
        if !propagated.match_in_progress() {
            // The first round never finished: the match hasn't really begun.
            propagated.reset_match();
        }
        msgs.extend(propagated.make_all_observers_into_players()?);
    ...
    pub fn destructively_redact_for_players(&mut self, players: &[PlayerID]) {
        ...
        if game_ongoing {
            self.hands.destructively_redact_except_for_players(players);
        }
        if game_ongoing && !players.contains(&self.exchanger) {
            self.kitty.fill(Card::Unknown);
        }
    }

    pub fn reset_requester(&self) -> Option<PlayerID> {

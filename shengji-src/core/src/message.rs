...
use crate::settings::{
    ...
    KittyPenalty, KittyTheftPolicy, MatchStanding, MultipleJoinPolicy, PlayTakebackPolicy,
    ThrowPenalty,
};
...
pub enum MessageVariant {
    ...
    /// A player clicked "start" for the first round of a match.
    StartVote {
        player: PlayerID,
        votes: usize,
        needed: usize,
    ...
    /// Somebody reached the target rank; standings are final.
    MatchFinished {
        match_key: String,
        first_to_rank: Rank,
        standings: Vec<MatchStanding>,
    },
    /// The roster changed mid-match; the climb starts over unrated.
    MatchAbandoned,
    /// The remaining cards can't change the result, so the round ended.
    GameEndedAutomatically,
...
impl MessageVariant {
    pub fn to_string<'a>(
        ...
        Ok(match self {
            ...
            RatedSet { rated: true } => format!("{} made this room rated", n?),
            RatedSet { rated: false } => format!("{} made this room unrated", n?),
            FirstToRankSet { rank } => format!("{} set the match to first to rank {}", n?, rank.as_str()),
            StartVote { player, votes, needed } =>
                format!("{} is ready to start ({votes}/{needed})", player_name(*player)?),
            MatchStarted { first_to_rank } => format!("The match has started: first to rank {}", first_to_rank.as_str()),
            MatchFinished { standings, first_to_rank, .. } => {
                let winners = standings.iter().filter(|s| s.winner).map(|s| s.name.as_str()).collect::<Vec<_>>();
                format!("The match is over: {} reached rank {}", winners.join(" and "), first_to_rank.as_str())
            }
            MatchAbandoned => "The match was abandoned because a player left; everyone is back at rank 2".to_string(),
            GameEndedAutomatically => "The remaining cards can't change the result, so the round is over".to_string(),

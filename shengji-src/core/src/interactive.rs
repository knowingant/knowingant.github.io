...
use crate::settings::{
    ...
    PlayerMode, PropagatedState, ThrowPenalty,
};
...
impl InteractiveGame {
    ...
    pub fn dump_state_for_players(&self, ids: &[PlayerID]) -> Result<GameState, Error> {
    ...
    pub fn set_player_mode(&mut self, player_mode: PlayerMode) -> Result<(), Error> {
        self.state.set_player_mode(player_mode)
    }
    ...
    pub fn interact(
        ...
        // Any change in the lobby invalidates the "everyone clicked start"
        // consensus for the first round of a match.
        if let GameState::Initialize(ref mut state) = self.state {
            if !matches!(msg, Action::StartGame) {
                state.clear_start_votes();
            }
        }
        ...
        let msgs = match (msg, &mut self.state) {
            ...
            (Action::StartGame, GameState::Initialize(ref mut state)) => {
                let mut msgs = vec![];
                let new_match = state.awaiting_start_votes();
                if new_match {
                    let (votes, needed) = state.vote_to_start(id)?;
                    if !state.all_players_voted_to_start() {
                        info!(logger, "Start vote"; "votes" => votes, "needed" => needed);
                        return self.hydrate_messages(
                            id,
                            vec![MessageVariant::StartVote {
                                player: id,
                                votes,
                                needed,
                            }],
                ...
                if new_match {
                    ...
                    state.begin_match(key);
                    msgs.push(MessageVariant::MatchStarted {
                        first_to_rank: state.first_to_rank(),
                    });
                }
                let mut draw = draw;
                draw.propagated_mut()
                    .begin_match(state.match_key().to_string());
                self.state = GameState::Draw(draw);
                msgs.push(MessageVariant::StartingGame);
                msgs
            }
            (Action::ReorderPlayers(ref players), GameState::Initialize(ref mut state)) => {
                info!(logger, "Reordering players");
                state.assert_settings_unlocked()?;
            ...
            (Action::SetRated(rated), GameState::Initialize(ref mut state)) => {
                info!(logger, "Setting rated"; "rated" => rated);
                state.assert_settings_unlocked()?;
                state.set_rated(rated)?
            }
            (Action::SetFirstToRank(rank), GameState::Initialize(ref mut state)) => {
                info!(logger, "Setting first-to rank"; "rank" => rank.as_str());
                state.assert_settings_unlocked()?;
                state.set_first_to_rank(rank)?
            }
...
pub enum Action {
    ...
    SetRated(bool),
    /// "First to rank N": the match ends when a player reaches this rank.
    SetFirstToRank(Rank),
    StartGame,
    DrawCard,
    RevealCard,

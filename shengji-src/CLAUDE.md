...
This directory is the knowingant **fork** of rbtying/shengji: accounts (SQLite), rated rounds, a 1v1 mode, and a static frontend on GitHub Pages (`../shengji/`) talking to a separately hosted backend. Read `DESIGN.md` (architecture + protocol), `RATINGS.md` (rating system) and `DEPLOY.md` (hosting, env vars) before changing anything. Fork-specific code: `rating/`, `backend/src/{db,auth,ratings,config}.rs`, `frontend/src/{api.ts,Landing.tsx,Auth.tsx,Game.tsx,SeatProvider.tsx,...}`. Rebuild and publish the frontend with `../scripts/build_shengji.sh`.

...
impl Rank {
    /// Position in the climb: 2 → 0, 3 → 1, …, A → 12, NT → 13.
    pub fn index(self) -> usize {
        match self {
            Rank::Number(n) => {
                let mut i = 0;
                let mut cur = Number::Two;
                while cur != n {
                    i += 1;
                    cur = match cur.successor() {
                        Some(next) => next,
                        None => return i,
                    };
                }
    ...
    /// Inverse of [`Rank::index`] (values above 13 map to NT).
    pub fn from_index(index: usize) -> Rank {
        let mut cur = Number::Two;
        for _ in 0..index {
            cur = match cur.successor() {
                Some(next) => next,
                None => return Rank::NoTrump,
            };
        }
        Rank::Number(cur)
    }
